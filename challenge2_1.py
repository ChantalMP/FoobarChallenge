...
and returns the maximum product of some non-empty subset of those numbers. So for example, if an array contained panels with power output
levels of [2, -3, 1, 0, -5], then the maximum product would be found by taking the subset: xs[0] = 2, xs[1] = -3, xs[4] = -5,
...
negative-output panels to produce the positive output of the multiple of their power values). The final products may be very large,
...
def solution(xs):
    ...
    if len(neg_numbers) % 2 != 0:
        neg_numbers = sorted(neg_numbers)
        neg_numbers = neg_numbers[0:-1]

    used_numbers.extend(neg_numbers)
